...
def volgen(
    ...
    while True:
        ...
        imgs = [voxelmorph.py.utils.load_volfile(vol_names[i], **load_params) for i in indices]
        vols = [np.concatenate(imgs, axis=0)]

        # optionally load segmentations and concatenate
        if return_segs:
            load_params['np_var'] = 'seg'  # be sure to load seg
            segs = [voxelmorph.py.utils.load_volfile(vol_names[i], **load_params) for i in indices]
...
def semisupervised(vol_names, labels, atlas_file=None, downsize=2):
    ...
    if atlas_file:
        trg_vol = voxelmorph.py.utils.load_volfile(atlas_file, np_var='vol', add_batch_axis=True, add_feat_axis=True)
        trg_seg = voxelmorph.py.utils.load_volfile(atlas_file, np_var='seg', add_batch_axis=True, add_feat_axis=True)
...
def conditional_template_creation(vol_names, atlas, attributes, batch_size=1, np_var='vol', pad_shape=None, add_feat_axis=True):
    ...
    while True:
        ...
        vols = [voxelmorph.py.utils.load_volfile(vol_names[i], **load_params) for i in indices]
