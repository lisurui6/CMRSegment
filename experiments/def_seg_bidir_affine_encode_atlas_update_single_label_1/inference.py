...
from experiments.def_seg_bidir_affine_encode_atlas_update_single_label_1.network import ImgTemplateEncoderNet
from experiments.def_seg_bidir_affine_encode_atlas_update_single_label_1.data import DefSegDataset
...
def inference(image: torch.Tensor, label: torch.Tensor, image_path: Path, network: torch.nn.Module, output_dir: Path, atlas):
    image, template = image
    # warped_template, warped_maps, pred_maps, flow = network((image, template))
    # warped_template, warped_maps, pred_maps, flow = network((image, template))
    affine_warped_template, warped_template, pred_maps, flow, warped_image, warped_label, __, __ = network((image, template), atlas)

    # for prefix, predicted in zip(["warped_template", "pred_maps"], [warped_template, pred_maps]):
    nim = nib.load(str(image_path))

    for prefix, predicted in zip(["warped_template", "affine_warped_template", "pred_maps"], [warped_template, affine_warped_template, pred_maps]):
