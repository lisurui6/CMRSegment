...
def inference(image: torch.Tensor, label: torch.Tensor, image_path: Path, network: torch.nn.Module, output_dir: Path):
    ...
    warped_template, affine_template, flow = network((image, template))
    for prefix, predicted in zip(["warped_template", "affine_template"], [warped_template, affine_template]):
