...
from experiments.geoshape.sample.ellipse import sample_lv_rv_points
...
class EllipseShapeDeformNet(torch.nn.Module):
    def __init__(self, voxel_width, voxel_depth, voxel_height, num_lv_slices, num_extra_slices, enc_dim, stages=None):
        if stages is None:
    ...
    def forward(self, img, epoch=0, label=None):
        ...
        [nodes0, tetras0, self.tri0], [nodes1, tetras1, self.tri1], [nodes2, tetras2, self.tri2] = sample_lv_rv_points(
            lv_par1=lv_par1, lv_par2=lv_par2, rv_par1=rv_par_sig, num_lv_slices=self.num_lv_slices,
...
def display(init_mask0, init_mask1, init_mask2, label):
    from mayavi import mlab
    from CMRSegment.common.nn.torch.augmentation import resize_image
    opacity = 0.999
    label = torch.movedim(label, 2, -1)

    print(init_mask0.shape, label.shape)
    label = label.detach().cpu().numpy()[0]
    lv_label = np.zeros_like(label[0])
    lv_label[[label[0] == 1]] = 1  # 1: LV endo, 2: LV myo, 3,4: RV
    lv_myo_label = np.zeros_like(label[0])
    lv_myo_label[[label[1] == 1]] = 1
    rv_label = np.zeros_like(label[0])
    rv_label[[label[2] == 1]] = 1

    for masks, prefix in zip([[init_mask0, init_mask1, init_mask2]], ["init"]):
        print(prefix)
        map = masks[1][0, 0].detach().cpu().numpy()
        xx, yy, zz = np.where(map > 0.5)
